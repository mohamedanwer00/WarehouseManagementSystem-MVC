$(document).ready(function () {
    ...
    // 1. تحديث الروابط لتستهدف SalesInvoices وسعر البيع
    const urls = {
        getProducts: '/SalesInvoices/GetProducts',
        getWarehouses: '/SalesInvoices/GetWarehouses',
        getCashBoxes: '/SalesInvoices/GetCashBoxes',
        getUnits: '/SalesInvoices/GetUnits',
        getSellingPrice: '/SalesInvoices/GetSellingPrice'
    };
    ...
    function buildRowHtml(index, item) {
        ...
        return `
            <tr>
                ...
                <td>
                    <input type="number" step="0.01" min="0" name="Items[${index}].SellingPrice" class="form-control text-center price-input" value="${sellingPrice}" />
                </td>
                <td>
    ...
    // تحميل وحدات المنتج واختيار الوحدة الأساسية تلقائياً
    ...
    // جلب سعر البيع عند اختيار الوحدة
    $(document).on("change", ".unit-select", function () {
        ...
        $.get(urls.getSellingPrice, { productUnitId: unitId }, function (price) {
    ...
    function reindexRows() {
        $("#itemsTable tbody tr").each(function (i, row) {
            ...
            $(row).find(".price-input").attr("name", `Items[${i}].SellingPrice`); // تم التعديل لـ SellingPrice
